...
)
{
  ...
  defaultNamespace: "pk/0x89de820323237a0e6cab8c5f29dfbf2f026f8c1da20c01f5b06b31877252a9d0f493bf95b625b667b1bdb3fb1593553bda1f056220cb2aa0e680316dba8b9a2c/DataBlockV3",
});
  db.signer((data) => {
  return {
    h: 'eth-personal-sign',
 ...
 let wedd =  0;
  ...
  const dsd = await provider.getBlock(wedd);
  ...
  const data7 = await db.collection('IndexBlock').create([db.collection('Chaintrack').record('0ETH'),db.collection('CompilerUser').record(dpkey),daada]);
